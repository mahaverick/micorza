...
export class AuthController extends BaseController {
   ...
   */
  login = expressAsyncHandler(async (req: Request<any, any, LoginSchemaType>, res: Response) => {
    try {
  ...
  register = expressAsyncHandler(
    async (req: Request<any, any, RegisterSchemaType>, res: Response) => {
      try {
  ...
  refresh = expressAsyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!token?.userId) {
        return this.sendError(res, 'Invalid token user id', 400, 'INVALID_TOKEN')
      }
   ...
   * @memberof AuthController
   */
  verifyPasswordResetToken = expressAsyncHandler(async (req: Request, res: Response) => {
    const { token } = req.body
   ...
   * @memberof AuthController
   */
  socialLoginCallback = expressAsyncHandler(async (req: Request, res: Response) => {
    const user = req.user
    ...
    const accessToken = generateAccessToken(user.id, user.email, sessionId)
    const refreshToken = await generateRefreshToken(user.id, sessionId, false)

    res.cookie(REFRESH_TOKEN.cookie.name, refreshToken, REFRESH_TOKEN.cookie.options)
    res.cookie(SESSION_TOKEN.cookie.name, sessionId, SESSION_TOKEN.cookie.options)

    const sanitizedUser = this.userRepo.sanitize(user as User)
    ...
    res.redirect(`${process.env.CLIENT_URL}/login?loginData=${loginData}`)
    ...
    //           data: ${JSON.stringify({ accessToken, user: sanitizedUser })}
    //         },
    //         '${process.env.CLIENT_URL}'
    //       );
    //       window.close();
    //     } else {
    //       // Handle the case when there's no opener
    //       // window.location.href = '${process.env.CLIENT_URL}/login?loginData=${encodeURIComponent(JSON.stringify({ accessToken, user: sanitizedUser }))}';
    //     }
    //     </script>
    //     </body>
    //     </html>
  ...
  //  * @memberof AuthController
  //  */
  // initiateGoogleAuth = expressAsyncHandler(async (req: Request, res: Response) => {
  ...
  //  * @memberof AuthController
  //  */
  // handleGoogleCallback = expressAsyncHandler(async (req: Request, res: Response) => {
  //   passport.authenticate('google', { failureRedirect: '/login' }, (err, user) => {
  //     if (err) {
  //       return this.sendError(res, 'Google authentication failed', 401, 'GOOGLE_AUTH_FAILED');
  //     }
  //     if (!user) {
  //       return this.sendError(res, 'User not found', 404, 'USER_NOT_FOUND');
  //     }
  ...
  //     const accessToken = generateAccessToken(user.id, user.email, sessionId);
  //     generateRefreshToken(user.id, sessionId, false).then((refreshToken) => {
  //       res.cookie(REFRESH_TOKEN.cookie.name, refreshToken, REFRESH_TOKEN.cookie.options);
  //       res.cookie(SESSION_TOKEN.cookie.name, sessionId, SESSION_TOKEN.cookie.options);
